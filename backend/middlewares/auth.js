...
const authenticateUserWithToken = async (req, res, next) => {
    try {
        ...
        if (!auth) throwUnAuthenticatedError(messages.ACCESS_DENIED);

        const authParts = auth.split(' ');
        if (authParts.length !== 2) throwUnAuthenticatedError('Format is: Bearer <token>');
        ...
        if (new RegExp('^Bearer$').test(scheme)) {
            try {
                ...
                throwUnAuthenticatedError(error.message);
            }
        } else {
